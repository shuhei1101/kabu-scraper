...
from kabuscraper.config_manager.stock_xlsx_handler import StockXlsxHandler
from kabuscraper.logger.my_logger import MyLogger
from kabuscraper.kabu_scraper import KabuScraper
...
async def main():
    ...
    try:
        ...
        if not os.path.exists(xlsx_path):
            print(
                f"`{os.getenv("XLSX_PATH")}`にファイルが存在しません。.envを確認してください。"
            )
            print("処理を終了します。")
            return
        ...
        handler = StockXlsxHandler(
            xlsx_path=xlsx_path,
            key=os.getenv("XLSX_KEY"),
        )
        nos = handler.get_keys()
        tasks = [process_row(no, handler) for no in nos if isinstance(no, float)]
...
async def process_row(no, handler: StockXlsxHandler):
    ...
    try:
        ...
        tasks = [
            process_column(column, record["銘柄コード"], scraper)
...
async def process_column(column, stock_code, scraper: KabuScraper):
    ...
    try:
        MyLogger().debug(
            f"銘柄コード '{stock_code}' の '{column}' をスクレイピング中..."
        )
        ...
        MyLogger().debug(
            f"'{stock_code}' の '{column}' のスクレイピングに失敗しました。空白で初期化します。"
        )
        return column, ""
    ...
    MyLogger().debug(
        f"銘柄コード'{stock_code}' の '{column}' を '{value}' に更新しました。"
    )
    return column, value
...
def traceback_to_json(exc_type, exc_value, exc_tb):
    ...
    return {"type": str(exc_type), "value": str(exc_value), "traceback": stack_trace}
